...
typedef struct enginefuncs_s
{
	...
	*/
	void		(*pfnSetSize)				(edict_t* pEdict, const float * vecMin, const float * vecMax);
	...
	*	@return Yaw angle.
	*/
	float		(*pfnVecToYaw)				(const float * vecVector);
	...
	*	@param vecOut Angles.
	*/
	void		(*pfnVecToAngles)			(const float * vecIn, float* vecOut);
	...
	*/
	void		(*pfnMoveToOrigin)			(edict_t* pEntity, const float * vecGoal, float dist, int iMoveType);
	...
	*/
	edict_t* (*pfnFindEntityInSphere)	(edict_t* pEdictStartSearchAfter, const float * vecOrigin, float rad);
	...
	*/
	void		(*pfnMakeVectors)			(const float * vecVector);
	...
	*/
	void		(*pfnAngleVectors)			(const float * vecVector, float* forward, float* right, float* up);
	...
	*/
	void		(*pfnSetOrigin)				(edict_t* pEntity, const float * vecOrigin);
	...
	*	@param pitch Sound pitch. Must be a value in the range [ 0, 255 ].
	*/
	void		(*pfnEmitAmbientSound)		(edict_t* pEntity, const float * vecPos, const char* pszSample, float vol, float attenuation, int fFlags, int pitch);
	...
	*/
	void		(*pfnTraceLine)				(const float * vecStart, const float * vecEnd, int fNoMonsters, edict_t* pentToSkip, TraceResult* ptr);
	...
	*/
	int			(*pfnTraceMonsterHull)		(edict_t* pEntity, const float * vecStart, const float * vecEnd, int fNoMonsters, edict_t* pentToSkip, TraceResult* ptr);
	...
	*	@param ptr TraceResult instance.
	*/
	void		(*pfnTraceHull)				(const float * vecStart, const float * vecEnd, int fNoMonsters, int hullNumber, edict_t* pentToSkip, TraceResult* ptr);
	...
	*	@param ptr TraceResult instance.
	*/
	void		(*pfnTraceModel)			(const float * vecStart, const float * vecEnd, int hullNumber, edict_t* pEntity, TraceResult* ptr);
	...
	*/
	const char* (*pfnTraceTexture)			(edict_t* pTextureEntity, const float * vecStart, const float * vecEnd);

	/**
	*	Not implemented. Triggers a sys error.
	*/
	void		(*pfnTraceSphere)			(const float * vecStart, const float * vecEnd, int fNoMonsters, float radius, edict_t* pentToSkip, TraceResult* ptr);
	...
	*/
	void		(*pfnGetAimVector)			(edict_t* pEntity, float speed, float* vecReturn);
	...
	*/
	void		(*pfnParticleEffect)		(const float * vecOrigin, const float * vecDir, float color, float count);
	...
	*/
	int			(*pfnPointContents)			(const float * vecVector);
	...
	*/
	void		(*pfnGetBonePosition)		(const edict_t* pEdict, int iBone, float* vecOrigin, float* vecAngles);
	...
	*/
	void		(*pfnGetAttachment)			(const edict_t* pEdict, int iAttachment, float* vecOrigin, float* vecAngles);
	...
	*/
	void		(*pfnRunPlayerMove)			(edict_t* fakeclient, const float * vecViewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, byte msec);
	...
	*/
	void		(*pfnStaticDecal)			(const float * vecOrigin, int decalIndex, int entityIndex, int modelIndex);
	...
	*/
	void		(*pfnBuildSoundMsg)			(edict_t* pEntity, int channel, const char* pszSample, float volume, float attenuation, int fFlags, int pitch, int iMsgType, int iMsgID, const float * vecOrigin, edict_t* ed);
	...
	*	@return PVS data.
	*/
	unsigned char* (*pfnSetFatPVS)			(const float * vecOrigin);

	/**
	*	Sets the Fat Potentially Audible Set buffer to contain data based on the given origin.
	*	@return PAS data.
	*/
	unsigned char* (*pfnSetFatPAS)			(const float * vecOrigin);
	...
	*/
	void		(*pfnDeltaSetField)			(struct delta_s* pFields, const char* pszFieldName);
	...
	*	@param pszFieldName Field name.
	*/
	void		(*pfnDeltaUnsetField)		(struct delta_s* pFields, const char* pszFieldName);
	...
	*/
	void		(*pfnDeltaAddEncoder)		(const char* const pszName, void (*conditionalencode)(struct delta_s* pFields, const unsigned char* from, const unsigned char* to));
	...
	*/
	int			(*pfnDeltaFindField)		(struct delta_s* pFields, const char* pszFieldName);
...
//	const char *(*pfnKeyNameForBinding)		( const char* pBinding );
	...
	*/
	sequenceEntry_s* (*pfnSequencePickSentence)	(const char* pszGroupName, int pickMethod, int* piPicked);
...
typedef struct 
{
	...
	*/
	char			(*pfnPM_FindTextureType)(char* pszName);
	...
	*/
	void			(*pfnCreateBaseline) (int player, int eindex, struct entity_state_s* baseline, edict_t* entity, int playermodelindex,
		const vec3_t player_mins, const vec3_t player_maxs);
