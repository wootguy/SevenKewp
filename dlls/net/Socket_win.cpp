...
Socket::Socket(int socketType, uint16_t port)
{
	...
	{
		if (bind())
			ALERT(at_console, "Server socket created on port %d\n", port);
	}
}
...
bool Socket::connect(uint32_t timeout)
{
	...
	if (ret)
	{
		...
		if (err == WSAEWOULDBLOCK)
		{
			...
			{
				ALERT(at_console, "Failed to connect socket: %d\n", err);
				return false;
			}
		}
...
Socket * Socket::accept()
{
	...
	if (newSock == INVALID_SOCKET)
	{
		...
		ALERT(at_console, "Socket accept failed: %d\n", err);
		return NULL;
	}
...
bool Socket::bind()
{
	if (skt == NULL)
	{
		ALERT(at_console, "Can't bind. Invalid socket!\n");
		return false;
	}
	SocketData * sock = (SocketData*)skt;
	...
	{
		ALERT(at_console, "Socket bind failed with error: %d\n", WSAGetLastError());
		closesocket(sock->sock);
	...
	if (socketType & SOCKET_TCP)
	{
		...
		if (iResult == SOCKET_ERROR)
		{
			ALERT(at_console, "Socket listen failed with error: %d\n", WSAGetLastError());
			closesocket(sock->sock);
			sock->sock = INVALID_SOCKET;
			return false;
		}
...
bool Socket::recv(Packet& p)
{
	...
	if (skt == NULL)
	{
		ALERT(at_console, "Can't receive. Invalid socket!\n");
		return false;
	}
	SocketData * sock = (SocketData*)skt;
	...
	if (packetLen == -1)
	{
		...
		if (err == WSAEWOULDBLOCK)
			return false;
		ALERT(at_console, "Socket recv failed with error: %d\n", err);
		return false;
	}
...
bool Socket::send( const Packet& p )
{
	...
	if ( ret < 0 )
	{
		...
		if (err == WSAEWOULDBLOCK)
			return false;
		ALERT(at_console, "Socket send failed with error: %d\n", err);
		return false;
	}
