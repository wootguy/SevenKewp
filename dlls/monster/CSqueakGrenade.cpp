...
void CSqueakGrenade :: Spawn( void )
{
	...
	pev->health			= gSkillData.sk_snark_health;
	pev->gravity		= 0.5;
	pev->friction		= 0.5;
...
void CSqueakGrenade::SuperBounceTouch( CBaseEntity *pOther )
{
	...
	if (pOther->pev->takedamage && m_flNextAttack < gpGlobals->time && (pOther->pev->flags & FL_WORLDBRUSH) == 0)
	{
		...
		if (tr.pHit == pOther->edict())
		{
			...
			{
				CBaseMonster* owner = m_hOwner ? m_hOwner->MyMonsterPointer() : NULL;
				...
				pev->dmg += gSkillData.sk_snark_dmg_pop; // add more explosion damage
