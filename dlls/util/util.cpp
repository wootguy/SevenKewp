...
HashMap<int> g_merged_models = {
	{ "models/w_9mmar.mdl",	MERGE_MDL_W_9MMAR},
	{ "models/w_9mmarclip.mdl", MERGE_MDL_W_9MMARCLIP},
	{ "models/w_9mmclip.mdl", MERGE_MDL_W_9MMCLIP},
	{ "models/w_9mmhandgun.mdl", MERGE_MDL_W_9MMHANDGUN},
	{ "models/w_357.mdl", MERGE_MDL_W_357},
	{ "models/w_357ammobox.mdl", MERGE_MDL_W_357AMMOBOX},
	{ "models/w_argrenade.mdl", MERGE_MDL_W_ARGRENADE},
	{ "models/w_battery.mdl", MERGE_MDL_W_BATTERY},
	{ "models/w_bgrap.mdl", MERGE_MDL_W_BGRAP},
	{ "models/w_chainammo.mdl", MERGE_MDL_W_CHAINAMMO},
	{ "models/w_crossbow.mdl", MERGE_MDL_W_CROSSBOW},
	{ "models/w_crossbow_clip.mdl", MERGE_MDL_W_CROSSBOW_CLIP},
	{ "models/w_crowbar.mdl", MERGE_MDL_W_CROWBAR},
	{ "models/w_displacer.mdl", MERGE_MDL_W_DISPLACER},
	{ "models/w_egon.mdl", MERGE_MDL_W_EGON},
	{ "models/w_gauss.mdl", MERGE_MDL_W_GAUSS},
	{ "models/w_gaussammo.mdl", MERGE_MDL_W_GAUSSAMMO},
	{ "models/w_grenade.mdl", MERGE_MDL_W_GRENADE},
	{ "models/w_hgun.mdl", MERGE_MDL_W_HGUN},
	{ "models/w_longjump.mdl", MERGE_MDL_W_LONGJUMP},
	{ "models/w_medkit.mdl", MERGE_MDL_W_MEDKIT},
	{ "models/w_pipe_wrench.mdl", MERGE_MDL_W_PIPE_WRENCH},
	{ "models/w_rpg.mdl", MERGE_MDL_W_RPG},
	{ "models/w_rpgammo.mdl", MERGE_MDL_W_RPGAMMO},
	{ "models/w_satchel.mdl", MERGE_MDL_W_SATCHEL},
	{ "models/w_security.mdl", MERGE_MDL_W_SECURITY},
	{ "models/w_shotbox.mdl", MERGE_MDL_W_SHOTBOX},
	{ "models/w_shotgun.mdl", MERGE_MDL_W_SHOTGUN},
	{ "models/w_shotshell.mdl", MERGE_MDL_W_SHOTSHELL},
	{ "models/w_suit.mdl", MERGE_MDL_W_SUIT},
	{ "models/w_weaponbox.mdl", MERGE_MDL_W_WEAPONBOX},
	{ "models/grenade.mdl", MERGE_MDL_GRENADE},
	{ "models/hvr.mdl", MERGE_MDL_HVR},
	{ "models/rpgrocket.mdl", MERGE_MDL_RPGROCKET},
	{ "models/spore.mdl", MERGE_MDL_SPORE},
	{ "models/shock_effect.mdl", MERGE_MDL_SHOCK_EFFECT},
	{ "models/w_2uzis.mdl", MERGE_MDL_W_2UZIS},
	{ "models/w_uzi.mdl", MERGE_MDL_W_UZI},
	{ "models/w_uzi_clip.mdl", MERGE_MDL_W_UZI_CLIP},
	{ "models/w_saw.mdl", MERGE_MDL_W_SAW},
	{ "models/w_saw_clip.mdl", MERGE_MDL_W_SAW_CLIP},
	{ "models/w_desert_eagle.mdl", MERGE_MDL_W_DESERT_EAGLE},
	{ "models/w_m40a1.mdl", MERGE_MDL_W_M40A1},
	{ "models/w_m40a1clip.mdl", MERGE_MDL_W_M40A1_CLIP},
	{ "models/w_minigun.mdl", MERGE_MDL_W_MINIGUN},
	{ "models/w_knife.mdl", MERGE_MDL_W_KNIFE},
	{ "models/w_m16.mdl", MERGE_MDL_W_M16},
	{ "models/w_pmedkit.mdl", MERGE_MDL_W_PMEDKIT},
	{ "models/camera.mdl", MERGE_MDL_CAMERA},
};
...
void WriteHudElementParams(CBasePlayer* client, const hudelementparams_t& params, const char* sprite, bool numeric) {
	uint8_t msgfl = 0;

	uint32_t flags = params.flags;

	if (numeric)
		flags |= HUD_ELEM_IS_NUMERIC;
	else
		flags &= ~HUD_ELEM_IS_NUMERIC;

	if (params.color1.r || params.color1.g || params.color1.b)
		msgfl |= HUD_ELEM_MSG_COLOR1;
	if (params.color2.r || params.color2.g || params.color2.b)
		msgfl |= HUD_ELEM_MSG_COLOR2;
...
void SendHudSpriteMessage(CBasePlayer* client, const HUDSpriteParams& hparams, const char* sprite, bool reliable) {
	uint8_t msgfl = 0;

	const hudspriteparams_t& params = hparams.spr;
...
void UTIL_HudCustomSprite(CBasePlayer* client, const HUDSpriteParams& params, const char* sprite, bool reliable) {
	if (client && !client->IsSevenKewpClient())
		return;
	...
	if (client) {
		...
		for (int i = 1; i <= gpGlobals->maxClients; i++) {
			...
			if (plr && plr->IsSevenKewpClient())
				SendHudSpriteMessage(plr, params, sprite, reliable);
		}
	}
}
...
void SendHudNumericMessage(CBasePlayer* client, const HUDNumDisplayParams& hparams, const char* sprite, bool reliable) {
	...
	WRITE_LONG(params.value);
	WRITE_BYTE(params.defdigits);
	WRITE_BYTE(params.maxdigits);
...
void UTIL_HudNumDisplay(CBasePlayer* client, const HUDNumDisplayParams& params, bool reliable) {
	if (client && !client->IsSevenKewpClient())
		return;
	...
	if (client) {
		...
		for (int i = 1; i <= gpGlobals->maxClients; i++) {
			...
			if (plr && plr->IsSevenKewpClient())
				SendHudNumericMessage(plr, params, "number_", reliable);
		}
	}
}

void UTIL_HudUpdateNum(CBasePlayer* client, uint8_t channel, float newVal, bool reliable) {
	if (client && !client->IsSevenKewpClient())
		return;
	...
	if (client) {
		...
		for (int i = 1; i <= gpGlobals->maxClients; i++) {
			...
			if (plr && plr->IsSevenKewpClient())
				SendUpdateNumMessage(plr, channel, newVal, reliable);
		}
	}
}
...
int UTIL_SendMotdFromFile(CBasePlayer* plr, const char* title, const char* fpath) {
	...
	MESSAGE_BEGIN(MSG_ONE, gmsgServerName, NULL, plr->edict());
	WRITE_STRING(title);
	MESSAGE_END();
