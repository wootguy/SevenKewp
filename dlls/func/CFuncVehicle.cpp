...
void CFuncVehicle :: UpdateSound( void )
{
	...
	if (!m_soundPlaying)
	{
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, (char*)STRING(pev->noise), m_flVolume, ATTN_NORM, 0, (int) flpitch);
		m_soundPlaying = 1;
	} 
	else
	{
		// update pitch
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, (char*)STRING(pev->noise), m_flVolume, ATTN_NORM, SND_CHANGE_PITCH, (int) flpitch);
