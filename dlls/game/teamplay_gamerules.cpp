...
CHalfLifeTeamplay :: CHalfLifeTeamplay()
{
	...
	strcpy_safe( m_szTeamList, teamlist.string, TEAMPLAY_TEAMLISTLENGTH );
	...
	if ( pWorld && pWorld->v.team )
	{
		if ( teamoverride.value )
		{
			...
			{
				strcpy_safe( m_szTeamList, pTeamList, TEAMPLAY_TEAMLISTLENGTH );
			}
		}
	}
...
const char *CHalfLifeTeamplay::SetDefaultPlayerTeam( CBasePlayer *pPlayer )
{
	...
	strcpy_safe( pPlayer->m_szTeamName, mdls, TEAM_NAME_LENGTH );
...
void CHalfLifeTeamplay::InitHUD( CBasePlayer *pPlayer )
{
	...
	MESSAGE_BEGIN( MSG_ONE, g_umsg.TeamNames, NULL, pPlayer->edict() );  
	...
	UTIL_ClientPrint(pPlayer, print_chat, text );
	RecountTeams();
	...
	for ( i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		{
			MESSAGE_BEGIN( MSG_ONE, g_umsg.TeamInfo, NULL, pPlayer->edict() );
...
void CHalfLifeTeamplay::ChangePlayerTeam( CBasePlayer *pPlayer, const char *pTeamName, BOOL bKill, BOOL bGib )
{
	...
	// copy out the team name from the model
	strcpy_safe( pPlayer->m_szTeamName, pTeamName, TEAM_NAME_LENGTH );

	SetClientKeyValue( clientIndex, g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "model", pPlayer->m_szTeamName );
	SetClientKeyValue( clientIndex, g_engfuncs.pfnGetInfoKeyBuffer( pPlayer->edict() ), "team", pPlayer->m_szTeamName );

	// notify everyone's HUD of the team change
	MESSAGE_BEGIN( MSG_ALL, g_umsg.TeamInfo );
		WRITE_BYTE( clientIndex );
		WRITE_STRING( pPlayer->m_szTeamName );
	MESSAGE_END();

	MESSAGE_BEGIN( MSG_ALL, g_umsg.ScoreInfo );
		WRITE_BYTE( clientIndex );
...
void CHalfLifeTeamplay::ClientUserInfoChanged( CBasePlayer *pPlayer, char *infobuffer )
{
	...
	UTIL_ClientPrintAll(print_chat, text);

	UTIL_LogPlayerEvent(pPlayer->edict(), "joined team \"%s\"\n", pPlayer->m_szTeamName, mdls );
...
void CHalfLifeTeamplay::RecountTeams( bool bResendInfo )
{
	...
	while ( pName != NULL && *pName )
	{
		...
		{
			strcpy_safe( team_names[num_teams], pName, 16 );
			num_teams++;
		}
		pName = strtok( NULL, ";" );
	}
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( plr )
		{
			...
			if ( bResendInfo ) //Someone's info changed, let's send the team info again.
			{
				if ( plr && IsValidTeam( plr->TeamID() ) )
				{
					MESSAGE_BEGIN( MSG_ALL, g_umsg.TeamInfo, NULL );
						WRITE_BYTE( plr->entindex() );
						WRITE_STRING( plr->TeamID() );
					MESSAGE_END();
				}
