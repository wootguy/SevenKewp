...
void CM249::Precache()
{
	...
	AddEvent(WepEvt().Primary().PlaySound(shootSnd, CHAN_WEAPON, 1.0f, ATTN_NORM, 94, 109, DISTANT_556, WC_AIVOL_LOUD, 0));
	...
	AddEvent(WepEvt().PrimaryEven().EjectShell(m_iShell, TE_BOUNCE_SHELL, 9, -8, 2));
	AddEvent(WepEvt().PrimaryOdd().EjectShell(m_iLink, TE_BOUNCE_SHELL, 9, -8, 2));
